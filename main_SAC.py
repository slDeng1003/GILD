...
from algorithm import SAC_GILD
from algorithm import SAC_GILD_ws
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", default="SAC_GILD_ws")  # Policy name
    ...
    parser.add_argument('--policy_type', default="Gaussian") # Policy type for SAC : Gaussian | Deterministic (default: Gaussian)
    parser.add_argument('--alpha', type=float, default=0.2, metavar='G') # Temperature parameter α determines the relative importance of the entropy term against the reward (default: 0.2)
    parser.add_argument('--automatic_entropy_tuning', type=bool, default=False, metavar='G') # Automaically adjust \alpha (default: False)
    parser.add_argument("--warm_start_timesteps", default=1e4, type=float)  # warm start steps
    ...
    if 'ws' in args.method:
        policy = SAC_GILD_ws.SAC_GILD(state_dim, env.action_space, demo_traj, args)
    else:
        policy = SAC_GILD.SAC_GILD(state_dim, env.action_space, demo_traj, args)
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t < args.start_timesteps:
            ...
            action = policy.select_action(state)  # Sample action for SAC

        # Perform action
        next_state, reward, done, _ = env.step(action) 
        # print(f"env._goal in train_env: {env._goal}")
        ...
        # Save period models
        if args.save_models:
            if (t) % args.save_freq == 0 and t >= args.start_timesteps:
