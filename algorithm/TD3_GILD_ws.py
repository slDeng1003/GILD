...
from model import Actor, Critic_DoubleQ, GILD_Network

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


class TD3_GILD(object):
    def __init__(self, state_dim, action_dim, max_action, demo_traj, args):
        ...
        self.critic = Critic_DoubleQ(state_dim, action_dim, 256).to(device)
        self.critic_target = Critic_DoubleQ(state_dim, action_dim, 256).to(device)
        ...
        self.warm_start_timesteps = args.warm_start_timesteps
    ...
    def train(self, replay_buffer, batch_size):
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            # ==========================================================================
            # at warm start stage, RL + GILD
            # ==========================================================================

            if self.total_it < self.warm_start_timesteps:
                ...
                loss_rl = -self.critic.Q1(state, self.actor(state)).mean()
                ...
                # loss_gild = -self.gild_network(concat_output).mean() # test only
                    # Compute actor's total loss
                actor_loss = loss_rl + loss_gild
                    # Optimize the actor
                self.actor_optimizer.zero_grad()
                actor_loss.backward(retain_graph=True, create_graph=True)
                self.hotplug.update(self.lr_actor)

                # compute meta loss
                    # RL loss of actor_tmp (RL+BC)
                action_val_bc = actor_tmp(state_val)
                policy_loss_val_bc = -self.critic.Q1(state_val, action_val_bc).mean().detach() # -Q_bc
            ...
            # ==========================================================================
            # after warm start stage, RL only
            # ==========================================================================
            else:
                # Compute actor losse
